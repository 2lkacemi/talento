...
export default function RegisterAgencyPage() {
  ...
  const t = useTranslations("registerAgency");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const auth = await authApi.registerAgency(form.agencyName, form.fullName, form.email, form.password);
      saveAuth(auth);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">{t("agencyName")}</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">{ta("email")}</label>
              <input
                type="email"
                required
                value={form.email}
